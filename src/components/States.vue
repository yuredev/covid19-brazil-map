    ...
    <LCircle v-bind="states[key]" :key="key" v-for="(state, key) of states">
      <LPopup>
        <p>
          ...
          <br />
          Casos confirmados: {{state.confirmedCases}}
          <br />
          Mortes: {{state.deaths}}
        </p>
      </LPopup>
  ...
  methods: {
    setCovidInfo(statesArray) {
      ...
      for (let i = 0; i < statesArray.length; i++) {
        this.states[statesArray[i].uf.toLowerCase()].setConfirmedCases(
          statesArray[i].cases
        );
        this.states[statesArray[i].uf.toLowerCase()].deaths =
          statesArray[i].deaths;
        this.states[statesArray[i].uf.toLowerCase()].name =
          statesArray[i].state;
        this.states[statesArray[i].uf.toLowerCase()].uf = statesArray[i].uf;

        brazilCases += this.states[statesArray[i].uf.toLowerCase()]
          .confirmedCases;
